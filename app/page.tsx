...
export default function Home() {
  ...
  // Safety check for technical skills structure
  const safeSkills = {
    testing: technicalSkills?.testing || [],
    tools: technicalSkills?.tools || [],
    programming: technicalSkills?.programming || [],
    softSkills: technicalSkills?.softSkills || []
  }
  ...
  return (
    <main className="min-h-screen bg-black text-white">
      ...
      <div className="relative z-10 container mx-auto p-3 sm:p-4 pt-20 sm:pt-24 pb-6 sm:pb-8">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
                    <AnimatedSection animation="slide-right" delay={100}>
                      <div className="space-y-3">
                        <h4 className="text-sm font-medium text-zinc-400">Testing</h4>
                        <div className="flex flex-wrap gap-2">
                          {safeSkills.testing.map((skill, index) => (
                    ...
                    <AnimatedSection animation="slide-left" delay={200}>
                      <div className="space-y-3">
                        ...
                        <div className="flex flex-wrap gap-2">
                          {safeSkills.tools.map((skill, index) => (
                            <SkillTagComponent key={index}>{skill}</SkillTagComponent>
                          ))}
                        </div>
                      </div>
                    ...
                    <AnimatedSection animation="slide-right" delay={300}>
                      <div className="space-y-3">
                        ...
                        <div className="flex flex-wrap gap-2">
                          {safeSkills.programming.map((skill, index) => (
                            <SkillTagComponent key={index}>{skill}</SkillTagComponent>
                          ))}
                        </div>
                      </div>
                    ...
                    <AnimatedSection animation="slide-left" delay={400}>
                      <div className="space-y-3">
                        ...
                        <div className="flex flex-wrap gap-2">
                          {safeSkills.softSkills.map((skill, index) => (
                            <SkillTagComponent key={index}>{skill}</SkillTagComponent>
                          ))}
                        </div>
                      </div>
        ...
        >
          <p>© {new Date().getFullYear()} Antoniuos Samy Aziz. All rights reserved.</p>
        </AnimatedSection>
      </div>
